...
public class PerlinNoiseChunk : Chunk
{
    List<GameObject> cubes = new List<GameObject>();
    public float resolution = 8;

    public override void UpdateChunk(float time){
        base.UpdateChunk(time);
    }
    ...
    public override void GenerateChunk(){
        ...
        for(float i = 0; i < resolution; i+= size.x/resolution){    
            for(float j = 0; j < resolution; j+= size.y/resolution){
                for(float k = 0; k < resolution; k+= size.z/resolution){
                    ...
                    if(Mathf.Abs(Perlin.Noise(chunk.x + i/size.x, chunk.y + j/size.y, chunk.z + k/size.z)) > .5){
                        ...
                        go.transform.localScale = size/resolution;
                        go.transform.localPosition = new Vector3(i - resolution/2, j - resolution/2, k - resolution/2);
                        cubes.Add(go);
                    }
                }
            }
